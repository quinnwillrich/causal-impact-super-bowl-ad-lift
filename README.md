...
This isn't how I'd run a CausalImpact analysis if I had access to real business data. Two limitations are worth being upfront about.

**Google Trends is a pretty rough KPI.** It's a relative index, not an actual business outcome. In practice you'd want to measure something like website sessions, new customer signups, or retail sales, because those connect to revenue. Search interest moves around for all kinds of reasons that have nothing to do with your campaign, and even when it does move, it doesn't tell you whether anyone bought anything. It's what you use when you don't have anything better.

**Peer-brand controls are a workaround.** The more common setup for CausalImpact in marketing is geo-based: you hold out a set of markets from seeing the campaign and use them as your counterfactual. That works well because the control markets are subject to the same seasonality, category trends, and macro conditions as the treated ones. You're not trying to find a brand that behaves like your brand; you're using your own brand in unexposed markets. The reason this project uses peer brands instead is that a national Super Bowl buy doesn't leave any markets unexposed, so there's nothing to hold out. Peer-brand matching is the right call given that constraint, but it's also where most of the model's problems come from.
